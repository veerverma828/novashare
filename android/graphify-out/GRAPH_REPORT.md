...
## Corpus Check
- 21 files · ~17,062 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 173 nodes · 262 edges · 21 communities (20 shown, 1 thin omitted)
...
2. `HotspotPlugin` - 12 edges
3. `LocalSignalingServerPlugin` - 12 edges
4. `NearbyDiscoveryPlugin` - 12 edges
5. `AppUpdatePlugin` - 9 edges
6. `NotifyDownloadPlugin` - 9 edges
7. `IncomingSharePlugin` - 7 edges
8. `InstalledAppsPlugin` - 7 edges
9. `TransferNotificationPlugin` - 7 edges
10. `MainActivity` - 6 edges
...
### Community 20 - "HotspotPlugin"
Cohesion: 0.21
Nodes (6): HotspotPlugin, Plugin, PluginCall, ConnectivityManager, Network, WifiManager
...
- **Why does `WifiDirectPlugin` connect `WifiDirectPlugin` to `JSObject`?**
  _High betweenness centrality (0.107) - this node is a cross-community bridge._
- **Are the 20 inferred relationships involving `JSObject` (e.g. with `.checkForUpdate()` and `.handleOnActivityResult()`) actually correct?**
  _`JSObject` has 20 INFERRED edges - model-reasoned connections that need verification._